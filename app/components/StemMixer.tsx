...
const Crossfader = memo<CrossfaderProps>(({ value, onChange, disabled = false }) => {
    ...
    <div className="flex flex-col items-center w-full">
      ...
      <div className="relative w-full h-8 bg-gray-800 rounded-lg border border-gray-600 overflow-hidden">
        ...
        <div
          ...
          style={{
            left: `${value * 100}%`,
            borderColor: value < 0.5 ? stemColors.original : stemColors.melody
          }}
        >
          {/* Inner indicator */}
          <div
            ...
            style={{
              backgroundColor: value < 0.5 ? stemColors.original : stemColors.melody,
      ...
      <div className="text-xs text-gray-400 mt-2 font-mono">
        {value === 0.5 ? 'CENTER' : value < 0.5 ? `ORG ${Math.round((0.5 - value) * 200)}%` : `STM ${Math.round((value - 0.5) * 200)}%`}
      </div>
    </div>
  );
});
...
const StemMixerComponent: React.FC<StemMixerProps> = ({
  ...
  const { metrics, status, startMeasurement, endMeasurement } = useOptimization();

  const deck = decks[channel];
  const controls = stemControls[channel];

  // Mock sync status and levels for demonstration
  const [syncStatus, setSyncStatus] = useState<'synced' | 'drifting' | 'error'>('synced');
  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      if (deck.isPlaying) {
        ...
        setStemLevels({
          drums: controls.drums.muted ? 0 : controls.drums.volume * baseLevel * (0.8 + Math.random() * 0.4),
          bass: controls.bass.muted ? 0 : controls.bass.volume * baseLevel * (0.7 + Math.random() * 0.3),
          melody: controls.melody.muted ? 0 : controls.melody.volume * baseLevel * (0.6 + Math.random() * 0.4),
          vocals: controls.vocals.muted ? 0 : controls.vocals.volume * baseLevel * (0.9 + Math.random() * 0.2),
          original: controls.original.muted ? 0 : controls.original.volume * baseLevel * (0.8 + Math.random() * 0.3)
        });
      } else {
        setStemLevels({
          drums: 0,
  ...
  }, [deck.isPlaying, controls, status, startMeasurement, endMeasurement]);
  ...
  const handleStemMixChange = useCallback((mix: number) => {
  ...
  return (
    <div className={`bg-gray-900 rounded-lg p-4 border border-gray-700 ${className}`}>
      ...
      <div className="mb-6">
        ...
        <Crossfader
          value={controls.stemMix}
          onChange={handleStemMixChange}
        />
      </div>
