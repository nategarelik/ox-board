...
const VolumeSlider = memo<VolumeSliderProps>(
  ({ value, onChange, disabled = false, color }) => {
...
const EQKnob = memo<EQKnobProps>(
  ({ band, value, onChange, disabled = false, color }) => {
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <div
          ref={knobRef}
          role="button"
          aria-label={`${band.toUpperCase()} EQ control`}
          className={`
...
const PanKnob = memo<PanKnobProps>(
  ({ value, onChange, disabled = false, color }) => {
    const knobRef = useRef<HTMLDivElement>(null);
    const [isDragging, setIsDragging] = useState(false);
  ...
  return (
    <div className="w-3 h-24 bg-gray-800 rounded-sm border border-gray-700 overflow-hidden">
      {Array.from({ length: segments }, (_, i) => {
        ...
        const isPeak =
          peakLevel >= segmentLevel && peakLevel < segmentLevel + 1 / segments;
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-4 border border-gray-700 w-32">
      ...
      <div className="text-center mb-3">
        ...
        <div className="text-xs text-gray-500 capitalize">{stemType}</div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <button
          role="button"
          aria-label="Mute"
          aria-pressed={stemControl.muted}
        ...
        <button
          role="button"
          aria-label="Solo"
          aria-pressed={stemControl.soloed}
