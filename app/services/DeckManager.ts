...
import { Crossfader } from "@/lib/audio/crossfader";
...
export class DeckManager extends EventEmitter implements DeckManagerInterface {
  ...
  private constructor() {
    ...
    // Don't check AudioService here - this causes the race condition
    // The check will be done in initializeDecks() instead

    // Create decks (but don't initialize yet)
    ...
    // Create master chain nodes (but don't connect them yet)
    // These will be connected after AudioService is ready
    ...
    // Setup event listeners (doesn't require deck outputs)
    this.setupEventListeners();

    // NOTE: Performance monitoring will be started after initialization
    // NOTE: Audio chain connection moved to after deck initialization
  }
  ...
  /**
   * Initializes both decks asynchronously
   * @returns Promise that resolves when both decks are ready
   */
  ...
  private async _performInitialization(): Promise<void> {
    try {
      // Check AudioService before initializing decks
      const audioService = getAudioService();
      if (!audioService.isReady()) {
        throw new Error(
          "AudioService must be initialized before initializing decks",
        );
      }
      ...
      // Now connect the audio chain after decks are initialized
      this.connectAudioChain();

      // Start performance monitoring after everything is ready
  ...
  /**
   * Check if the DeckManager is initialized
   */
  ...
  private setupEventListeners(): void {
    ...
    this.deckB.on("loaded", (data) =>
      this.emit("deck:loaded", { deck: "B", ...data }),
    );
    this.deckB.on("cue", () => this.emit("deck:cue", { deck: "B" }));
    this.deckB.on("error", (error) =>
      this.emit("deck:error", { deck: "B", error }),
    );
  ...
  // Test Helper Methods
  public getDecks(): Array<{ id: number }> {
    return [
      { id: 1, ...this.deckA.getState() },
      { id: 2, ...this.deckB.getState() },
    ];
  }

  public getDeck(id: number): any {
    if (id === 1) {
      return { id: 1, ...this.deckA.getState() };
    } else if (id === 2) {
      return { id: 2, ...this.deckB.getState() };
    }
    return undefined;
  }

  public setDeckVolume(deckId: number, volume: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
      deck.setVolume(volume);
      this.emit("deck:volume", { deck: deckId === 1 ? "A" : "B", volume });
    }
  }

  public togglePlay(deckId: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
      if (deck.getState().isPlaying) {
        deck.pause();
      } else {
        deck.play();
      }
  ...
  public addCuePoint(deckId: number, position: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
      deck.setCuePoint(position);
    }
  }

  public setLoop(deckId: number, start: number, end: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
  ...
  public clearLoop(deckId: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
      deck.clearLoop();
    }
  ...
  public setPlaybackRate(deckId: number, rate: number): void {
    const deck = deckId === 1 ? this.deckA : deckId === 2 ? this.deckB : null;
    if (deck) {
      // Playback rate is controlled via pitch adjustment
      const pitchPercent = (rate - 1) * 100; // Convert rate to pitch percentage
      deck.setPitch(pitchPercent);
    }
  }
}
