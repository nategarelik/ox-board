...
import type {
  ...
  AudioAnalysisResult,
} from "../lib/workers/audioAnalyzer.worker";
...
const useStemPerformance = () => {
  ...
  const analyzeAudioData = useCallback(
    async (
      ...
      if (
        !workerPoolRef.current ||
        workerPoolRef.current.workers.length === 0
      ) {
      ...
      const worker =
        workerPoolRef.current.workers[workerPoolRef.current.currentWorkerIndex];

      // Round-robin worker selection
      workerPoolRef.current.currentWorkerIndex =
        (workerPoolRef.current.currentWorkerIndex + 1) %
        workerPoolRef.current.workers.length;
  ...
  const optimizePerformance = useCallback(() => {
    ...
    if (
      workerPoolRef.current &&
      workerPoolRef.current.activeRequests.size > 10
    ) {
      ...
      for (const [
        id,
        request,
      ] of workerPoolRef.current.activeRequests.entries()) {
        if (now - request.timestamp > 1000) {
          request.reject(
            new Error("Request cancelled due to performance optimization"),
          );
  ...
  const createVirtualScrollState = useCallback(
    (config: VirtualScrollConfig): VirtualScrollState => {
  ...
  const updateVirtualScrollState = useCallback(
    (config: VirtualScrollConfig, scrollTop: number): VirtualScrollState => {
      const { itemHeight, bufferSize, containerHeight, totalItems } = config;
      const visibleItems = Math.ceil(containerHeight / itemHeight);
