...
import type { CrossfaderConfig } from '@/app/services/types/audio'
...
export class Crossfader {
  ...
  public setPosition(position: number): void {
    ...
    position = Math.max(AUDIO_CONSTANTS.CROSSFADER_A,
  ...
  private updateGains(): void {
    const { gainA, gainB } = this.calculateGains(this.config.position, this.config.curve)
  ...
  private calculateGains(position: number, curve: CrossfaderConfig['curve']): { gainA: number, gainB: number } {
  ...
  public center(duration: number = 0): void {
    if (duration > 0) {
      this.animatePosition(this.config.position, AUDIO_CONSTANTS.CROSSFADER_CENTER, duration)
    } else {
