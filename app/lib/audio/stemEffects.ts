...
export type EffectType = 'reverb' | 'delay' | 'filter' | 'distortion' | 'compression' | 'phaser' | 'flanger';
...
export class StemEffectsProcessor {
  ...
  private performanceMetrics: Map<number, { cpu: number; latency: number }> = new Map();
  ...
  initializeStem(stemIndex: number): void {
    ...
    const effectTypes: EffectType[] = ['compression', 'filter', 'distortion', 'delay', 'reverb', 'phaser', 'flanger'];
  ...
  private connectEffectChain(stemIndex: number, routing: RoutingType): void {
    ...
    if (routing === 'serial') {
      ...
      const effectOrder: EffectType[] = ['compression', 'filter', 'distortion', 'delay', 'reverb', 'phaser', 'flanger'];
   ...
   */
  setEffectParameters(stemIndex: number, effectType: EffectType, params: EffectParameters): void {
    const effectChain = this.stemChains.get(stemIndex);
    if (!effectChain) {
  ...
  private areParametersDefault(effectType: EffectType, params: EffectParameters): boolean {
    ...
    switch (effectType) {
      ...
      case 'delay':
        return Math.abs(params.wetness - 0) < defaultThreshold;
      case 'filter':
        return Math.abs(params.parameter1 - 0.5) < defaultThreshold && Math.abs(params.parameter2 - 0.1) < defaultThreshold;
      case 'distortion':
        return Math.abs(params.wetness - 0) < defaultThreshold;
      case 'compression':
        return Math.abs(params.parameter1 - 0.4) < defaultThreshold && Math.abs(params.parameter2 - 0.2) < defaultThreshold;
      case 'phaser':
      case 'flanger':
        return Math.abs(params.wetness - 0) < defaultThreshold;
      default:
   ...
   */
  processGestureInput(stemIndex: number, gesture: string, value: number, secondary?: number): void {
    ...
    switch (gesture) {
      case 'pinch':
        ...
        if (mapping.effect && config.effects[mapping.effect]) {
          ...
          this.setEffectParameters(stemIndex, mapping.effect, params as EffectParameters);
        }
        break;
  ...
  private cycleEffect(stemIndex: number, direction: number): void {
    ...
    if (currentIndex >= 0) {
      ...
      this.setEffectParameters(stemIndex, currentEffect, config.effects[currentEffect] as EffectParameters);
    }

    // Enable next effect
    const nextIndex = (currentIndex + direction + effectTypes.length) % effectTypes.length;
    const nextEffect = effectTypes[nextIndex];
    config.effects[nextEffect].enabled = true;
    this.setEffectParameters(stemIndex, nextEffect, config.effects[nextEffect] as EffectParameters);
  }
  ...
  applyPreset(stemIndex: number, presetType: PresetType): void {
    ...
    Object.entries(preset.config.effects).forEach(([effectType, params]) => {
      this.setEffectParameters(stemIndex, effectType as EffectType, params as EffectParameters);
    });
  ...
  private initializePresets(): void {
    ...
    this.presets.set('club', {
      ...
      config: {
        ...
        effects: {
          compression: {
            enabled: true, wetness: 1.0, intensity: 0.7, parameter1: 0.3, parameter2: 0.4, bypass: false,
            threshold: 0.3, ratio: 0.4, attack: 0.01, release: 0.1
          },
          filter: {
            enabled: true, wetness: 1.0, intensity: 0.3, parameter1: 0.6, parameter2: 0.2, bypass: false,
            frequency: 0.6, resonance: 0.2, type: 'highpass' as FilterType
          },
          delay: {
            enabled: true, wetness: 0.2, intensity: 0.4, parameter1: 0.25, parameter2: 0.2, bypass: false,
            time: 0.125, feedback: 0.2, filter: 0.7
          },
          reverb: {
            enabled: false, wetness: 0.1, intensity: 0.3, parameter1: 0.3, parameter2: 0.4, bypass: false,
            roomSize: 0.3, decay: 0.8, preDelay: 0.005
          },
          distortion: {
            enabled: false, wetness: 0.3, intensity: 0.2, parameter1: 0.2, parameter2: 0.6, bypass: false,
            drive: 0.2, oversample: 0.5, tone: 0.6
          },
          phaser: {
            enabled: false, wetness: 0.3, intensity: 0.4, parameter1: 0.2, parameter2: 0.5, bypass: false,
            rate: 2.0, depth: 0.5, feedback: 0.1
          },
          flanger: {
            enabled: false, wetness: 0.25, intensity: 0.3, parameter1: 0.15, parameter2: 0.4, bypass: false,
            rate: 1.5, depth: 0.4, feedback: 0.05
          }
        }
      }
    });
    ...
    this.presets.set('hall', {
      ...
      config: {
        ...
        effects: {
          compression: {
            enabled: true, wetness: 1.0, intensity: 0.4, parameter1: 0.5, parameter2: 0.15, bypass: false,
            threshold: 0.5, ratio: 0.15, attack: 0.005, release: 0.3
          },
          reverb: {
            enabled: true, wetness: 0.6, intensity: 0.8, parameter1: 0.9, parameter2: 0.8, bypass: false,
            roomSize: 0.9, decay: 4.0, preDelay: 0.03
          },
          delay: {
            enabled: true, wetness: 0.15, intensity: 0.3, parameter1: 0.375, parameter2: 0.15, bypass: false,
            time: 0.375, feedback: 0.15, filter: 0.4
          },
          filter: {
            enabled: false, wetness: 1.0, intensity: 0.2, parameter1: 0.8, parameter2: 0.1, bypass: false,
            frequency: 0.8, resonance: 0.1, type: 'lowpass' as FilterType
          },
          distortion: {
            enabled: false, wetness: 0.1, intensity: 0.1, parameter1: 0.1, parameter2: 0.7, bypass: false,
            drive: 0.1, oversample: 0.25, tone: 0.7
          },
          phaser: {
            enabled: false, wetness: 0.2, intensity: 0.2, parameter1: 0.05, parameter2: 0.3, bypass: false,
            rate: 0.5, depth: 0.3, feedback: 0.05
          },
          flanger: {
            enabled: false, wetness: 0.15, intensity: 0.2, parameter1: 0.08, parameter2: 0.25, bypass: false,
            rate: 0.8, depth: 0.25, feedback: 0.03
          }
        }
      }
    });
    ...
    this.presets.set('studio', {
      ...
      config: {
        ...
        effects: {
          compression: {
            enabled: true, wetness: 1.0, intensity: 0.5, parameter1: 0.4, parameter2: 0.2, bypass: false,
            threshold: 0.4, ratio: 0.2, attack: 0.003, release: 0.25
          },
          filter: {
            enabled: true, wetness: 1.0, intensity: 0.2, parameter1: 0.45, parameter2: 0.05, bypass: false,
            frequency: 0.45, resonance: 0.05, type: 'highpass' as FilterType
          },
          reverb: {
            enabled: false, wetness: 0.05, intensity: 0.2, parameter1: 0.4, parameter2: 0.3, bypass: false,
            roomSize: 0.4, decay: 0.8, preDelay: 0.01
          },
          delay: {
            enabled: false, wetness: 0.08, intensity: 0.2, parameter1: 0.125, parameter2: 0.1, bypass: false,
            time: 0.125, feedback: 0.1, filter: 0.6
          },
          distortion: {
            enabled: false, wetness: 0.05, intensity: 0.1, parameter1: 0.05, parameter2: 0.8, bypass: false,
            drive: 0.05, oversample: 0.75, tone: 0.8
          },
          phaser: {
            enabled: false, wetness: 0.1, intensity: 0.15, parameter1: 0.1, parameter2: 0.2, bypass: false,
            rate: 1.0, depth: 0.2, feedback: 0.02
          },
          flanger: {
            enabled: false, wetness: 0.08, intensity: 0.12, parameter1: 0.12, parameter2: 0.18, bypass: false,
            rate: 1.2, depth: 0.18, feedback: 0.02
          }
        }
      }
    });
    ...
    this.presets.set('outdoor', {
      ...
      config: {
        ...
        effects: {
          compression: {
            enabled: true, wetness: 1.0, intensity: 0.8, parameter1: 0.25, parameter2: 0.5, bypass: false,
            threshold: 0.25, ratio: 0.5, attack: 0.002, release: 0.15
          },
          filter: {
            enabled: true, wetness: 1.0, intensity: 0.6, parameter1: 0.55, parameter2: 0.3, bypass: false,
            frequency: 0.55, resonance: 0.3, type: 'bandpass' as FilterType
          },
          delay: {
            enabled: true, wetness: 0.3, intensity: 0.5, parameter1: 0.25, parameter2: 0.25, bypass: false,
            time: 0.25, feedback: 0.25, filter: 0.5
          },
          reverb: {
            enabled: false, wetness: 0.1, intensity: 0.2, parameter1: 0.5, parameter2: 0.4, bypass: false,
            roomSize: 0.5, decay: 1.2, preDelay: 0.02
          },
          distortion: {
            enabled: false, wetness: 0.2, intensity: 0.3, parameter1: 0.15, parameter2: 0.5, bypass: false,
            drive: 0.15, oversample: 0.5, tone: 0.5
          },
          phaser: {
            enabled: false, wetness: 0.25, intensity: 0.3, parameter1: 0.3, parameter2: 0.4, bypass: false,
            rate: 3.0, depth: 0.4, feedback: 0.08
          },
          flanger: {
            enabled: false, wetness: 0.2, intensity: 0.25, parameter1: 0.25, parameter2: 0.35, bypass: false,
            rate: 2.5, depth: 0.35, feedback: 0.06
          }
        }
      }
    });
  ...
  private initializeGestureMapping(): void {
    this.gestureMapping = [
      { gesture: 'pinch', effect: 'reverb', parameter: 'intensity', sensitivity: 1.0, threshold: 0.1 },
      { gesture: 'rotation', effect: 'filter', parameter: 'parameter1', sensitivity: 1.0, threshold: 0.05 },
      { gesture: 'spread', effect: 'delay', parameter: 'wetness', sensitivity: 1.0, threshold: 0.1 },
      { gesture: 'tap', effect: 'distortion', parameter: 'enabled', sensitivity: 1.0, threshold: 0.8 },
      { gesture: 'swipe', effect: 'compression', parameter: 'cycle', sensitivity: 1.0, threshold: 0.3 }
    ];
  }
   ...
   */
  updateGestureMapping(gesture: string, effect: EffectType, parameter: string, sensitivity: number = 1.0, threshold: number = 0.1): void {
    const existingIndex = this.gestureMapping.findIndex(m => m.gesture === gesture);
