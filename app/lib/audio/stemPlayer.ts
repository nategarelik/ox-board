...
export class StemPlayer {
  ...
  async loadStems(demucsOutput: DemucsOutput): Promise<StemLoadResult[]> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error loading stems';
  ...
  setStemVolume(stemType: StemType | 'original', volume: number): void {
    try {
      ...
      this.emit('stemControlChanged', { stemType, control: 'volume', value: clampedVolume });
  ...
  setStemMute(stemType: StemType | 'original', muted: boolean): void {
    try {
      ...
      const targetVolume = muted ? 0 : (stemType === 'original' ? this.state.original.volume : this.state.stems[stemType].volume);
      this.gains[stemType].gain.rampTo(targetVolume, 0.01);

      this.emit('stemControlChanged', { stemType, control: 'mute', value: muted });
  ...
  setStemSolo(stemType: StemType | 'original', soloed: boolean): void {
    try {
      ...
      const allStems: (StemType | 'original')[] = ['drums', 'bass', 'melody', 'vocals', 'original'];
      const soloedStems = allStems.filter(stem =>
        stem === 'original' ? this.state.original.soloed : this.state.stems[stem as StemType].soloed
      );

      allStems.forEach(stem => {
        const isSoloed = soloedStems.length === 0 || soloedStems.includes(stem);
        const controls = stem === 'original' ? this.state.original : this.state.stems[stem as StemType];
        const targetVolume = (isSoloed && !controls.muted) ? controls.volume : 0;

        this.gains[stem].gain.rampTo(targetVolume, 0.01);
      });

      this.emit('stemControlChanged', { stemType, control: 'solo', value: soloed });
  ...
  setStemPan(stemType: StemType | 'original', pan: number): void {
    try {
      ...
      this.emit('stemControlChanged', { stemType, control: 'pan', value: clampedPan });
  ...
  setStemEQ(stemType: StemType | 'original', band: 'low' | 'mid' | 'high', value: number): void {
    try {
      ...
      this.emit('stemControlChanged', { stemType, control: 'eq', band, value: clampedValue });
  ...
  setStemPlaybackRate(stemType: StemType | 'original', rate: number): void {
    try {
      ...
      this.emit('stemControlChanged', { stemType, control: 'playbackRate', value: clampedRate });

    } catch (error) {
      this.emit('error', {
        error: error instanceof Error ? error.message : 'Unknown playback rate error',
  ...
  setStemMix(mix: number): void {
    try {
      ...
      this.emit('error', {
        error: error instanceof Error ? error.message : 'Unknown stem mix error',
        context: 'setStemMix'
      });
    }
  }
  ...
  private handlePlayerStop(): void {
    ...
    const activePlayers = Object.values(this.players).filter(p => p && p.state === 'started');
