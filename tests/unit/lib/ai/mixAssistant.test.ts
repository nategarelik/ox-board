...
import { IntelligentMixAssistant, TrackAnalysis, CompatibilityScore, TransitionRecommendation } from '../mixAssistant';

// Mock the MusicAnalyzer dependency
jest.mock('../../audio/musicAnalyzer', () => ({
...
describe('IntelligentMixAssistant', () => {
  ...
  beforeEach(async () => {
    ...
    for (let i = 0; i < mockAudioBuffer.length; i++) {
      ...
      mockAudioBuffer[i] = Math.sin(2 * Math.PI * 440 * i / 44100) * (0.5 + 0.3 * Math.sin(2 * Math.PI * i / 22050));
    }
  });
  ...
  describe('Track Analysis', () => {
    it('should analyze a track and return complete analysis', async () => {
      const analysis = await mixAssistant.analyzeTrack('test-track-1', mockAudioBuffer);
      ...
      analysis.mixPoints.forEach(point => {
        expect(point).toMatchObject({
          ...
          type: expect.stringMatching(/^(intro|outro|breakdown|drop|phrase_start|phrase_end)$/),
    ...
    it('should detect phrase structure correctly', async () => {
      const analysis = await mixAssistant.analyzeTrack('test-track-phrase', mockAudioBuffer);
      ...
      analysis.phrases.phrases.forEach(phrase => {
        ...
        expect(['intro', 'verse', 'chorus', 'bridge', 'breakdown', 'outro']).toContain(phrase.type);
      });
    });
  });
  ...
  describe('Camelot Wheel Harmonic Compatibility', () => {
    beforeEach(() => {
      ...
      const mockIsCompatibleKey = (key1: string, key2: string): boolean => {
        ...
        const camelotMap: Record<string, { position: number; type: 'A' | 'B' }> = {
        ...
        // Adjacent positions
        const posDiff = Math.min(Math.abs(c1.position - c2.position), 12 - Math.abs(c1.position - c2.position));
        return posDiff === 1;
      };

      (mixAssistant as any).musicAnalyzer.isCompatibleKey.mockImplementation(mockIsCompatibleKey);
    });

    it('should calculate perfect harmonic compatibility for same key', () => {
      const track1: Partial<TrackAnalysis> = {
        key: { key: 'C', scale: 'major', confidence: 0.9, chroma: new Float32Array(), keyStrength: 0.8 }
      };
      const track2: Partial<TrackAnalysis> = {
        key: { key: 'C', scale: 'major', confidence: 0.9, chroma: new Float32Array(), keyStrength: 0.8 }
      };

      const harmonic = (mixAssistant as any).calculateHarmonicCompatibility(track1.key, track2.key);
    ...
    it('should calculate high compatibility for relative major/minor', () => {
      ...
      const track2: Partial<TrackAnalysis> = {
        key: { key: 'A', scale: 'minor', confidence: 0.9, chroma: new Float32Array(), keyStrength: 0.8 }
      };
    ...
    it('should calculate good compatibility for adjacent Camelot positions', () => {
      ...
      const track2: Partial<TrackAnalysis> = {
        key: { key: 'G', scale: 'major', confidence: 0.9, chroma: new Float32Array(), keyStrength: 0.8 }
      };
    ...
    it('should calculate lower compatibility for distant keys', () => {
      ...
      const track2: Partial<TrackAnalysis> = {
        key: { key: 'F#', scale: 'major', confidence: 0.9, chroma: new Float32Array(), keyStrength: 0.8 }
      };
  ...
  describe('BPM Matching and Tempo Adjustment', () => {
    it('should calculate perfect rhythmic compatibility for same BPM', () => {
      const bpm1 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };
      const bpm2 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };

      const rhythmic = (mixAssistant as any).calculateRhythmicCompatibility(bpm1, bpm2);
      expect(rhythmic).toBe(1.0);
    });

    it('should calculate high compatibility within natural mixing range (±8%)', () => {
      const bpm1 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };
      const bpm2 = { bpm: 135, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] }; // +5.5%
    ...
    it('should calculate tempo adjustment correctly', () => {
      const bpm1 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };
      const bpm2 = { bpm: 140, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };

      const adjustment = (mixAssistant as any).calculateTempoAdjustment(bpm1, bpm2);
      ...
      expect(Math.abs(adjustment.adjustmentPercent - (12/128))).toBeLessThan(0.001); // 12 BPM difference
    });

    it('should recommend pitch_shift for natural range adjustments', () => {
      const bpm1 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };
      const bpm2 = { bpm: 132, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] }; // +3.1%
    ...
    it('should recommend sync_lock for large tempo differences', () => {
      const bpm1 = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };
      const bpm2 = { bpm: 170, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] }; // +32.8%
  ...
  describe('Energy Level Analysis', () => {
    it('should analyze energy profile correctly', async () => {
      const analysis = await mixAssistant.analyzeTrack('test-energy', mockAudioBuffer);

      expect(analysis.energyProfile.overall).toBeGreaterThanOrEqual(0);
      expect(analysis.energyProfile.overall).toBeLessThanOrEqual(1);
      expect(analysis.energyProfile.curve.length).toBeGreaterThan(0);
      expect(analysis.energyProfile.peakEnergy).toBeGreaterThanOrEqual(analysis.energyProfile.overall);
      ...
      ['intro', 'verse', 'chorus', 'breakdown', 'outro'].forEach(section => {
        const value = analysis.energyProfile[section as keyof typeof analysis.energyProfile];
    ...
    it('should calculate energetic compatibility correctly', () => {
      ...
      const compatibility = (mixAssistant as any).calculateEnergeticCompatibility(energy1, energy2);
  ...
  describe('Mix Point Detection', () => {
    it('should identify appropriate mix points', async () => {
      const analysis = await mixAssistant.analyzeTrack('test-mixpoints', mockAudioBuffer);
      ...
      for (let i = 1; i < analysis.mixPoints.length; i++) {
        expect(analysis.mixPoints[i].score).toBeLessThanOrEqual(analysis.mixPoints[i - 1].score);
      }
    });
    ...
    it('should find compatible mix points between tracks', async () => {
      ...
      const compatible = (mixAssistant as any).findCompatibleMixPoints(track1, track2);
  ...
  describe('Track Compatibility Scoring', () => {
    it('should calculate overall compatibility score', async () => {
      ...
      const compatibility = mixAssistant.calculateCompatibility('track1', 'track2');
  ...
  describe('Transition Recommendations', () => {
    ...
    it('should generate intelligent transition recommendation', () => {
      const recommendation = mixAssistant.generateTransition('track1', 'track2');
    ...
    it('should generate effect recommendations based on track characteristics', () => {
      ...
      recommendation.effects.forEach(effect => {
        expect(effect).toMatchObject({
          type: expect.stringMatching(/^(highpass|lowpass|reverb|delay|filter_sweep|echo|gate)$/),
    ...
    it('should provide real-time mix suggestions', () => {
      ...
      for (let i = 1; i < suggestions.length; i++) {
        ...
        expect(prev.confidence + prev.compatibility.overall)
  ...
  describe('Beat Matching', () => {
    it('should calculate optimal beat sync points', async () => {
      const track1 = await mixAssistant.analyzeTrack('sync-track-1', mockAudioBuffer);
      const track2 = await mixAssistant.analyzeTrack('sync-track-2', mockAudioBuffer);
  ...
  describe('Performance and Edge Cases', () => {
    ...
    it('should handle very short audio buffers', async () => {
      ...
      const analysis = await mixAssistant.analyzeTrack('short-track', shortBuffer);
    ...
    it('should handle concurrent track analyses', async () => {
      const promises = [];
      for (let i = 0; i < 5; i++) {
        promises.push(mixAssistant.analyzeTrack(`concurrent-${i}`, mockAudioBuffer));
      }
  ...
  describe('Algorithm Accuracy', () => {
    it('should maintain consistent analysis results for same input', async () => {
      const analysis1 = await mixAssistant.analyzeTrack('consistency-test-1', mockAudioBuffer);
      const analysis2 = await mixAssistant.analyzeTrack('consistency-test-2', mockAudioBuffer);
      ...
      expect(analysis1.energyProfile.overall).toBeCloseTo(analysis2.energyProfile.overall, 2);
    });
    ...
    it('should calculate crossfade times based on phrase structure', () => {
      ...
      const bpm = { bpm: 128, confidence: 0.9, beatGrid: [], downbeats: [], phase: 0, timeSignature: [4, 4] as [number, number] };

      const crossfadeTime = (mixAssistant as any).calculateCrossfadeTime(fromMixPoint, toMixPoint, bpm);
