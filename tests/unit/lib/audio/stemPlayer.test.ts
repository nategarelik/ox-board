...
import { StemPlayer, StemPlayerConfig } from "@/lib/audio/stemPlayer";
import { DemucsOutput, StemType } from "@/lib/audio/demucsProcessor";
...
describe("StemPlayer", () => {
  ...
  describe("Stem Loading", () => {
    ...
    it("should dispose old players when loading new stems", async () => {
      await stemPlayer.loadStems(mockDemucsOutput);
      const firstLoadResults = await stemPlayer.loadStems(
        createMockDemucsOutput(15),
      );
  ...
  describe("Stem Controls", () => {
    ...
    describe("Playback Rate Control", () => {
      ...
      it("should throw error when time stretching is disabled", () => {
        ...
        expect(() => player.setStemPlaybackRate("drums", 1.5)).toThrow(
          "Time stretching is disabled",
        );
        player.dispose();
      });
    });
  ...
  describe("Crossfading", () => {
    ...
    it("should apply crossfade curves correctly", () => {
      ...
      stemPlayer.setStemMix(0.5);
      expect(mockCrossFade.fade.rampTo).toHaveBeenCalledWith(
        expect.any(Number),
        0.01,
      );
    });
