...
import { DefaultDemucsProcessor, DemucsOutput, StemData } from '../demucsProcessor';
...
const createMockFile = (name: string = 'test.mp3', size: number = 1024000): File => {
...
describe('DemucsProcessor', () => {
  ...
  beforeEach(() => {
    ...
    mockAudioContext.decodeAudioData.mockResolvedValue(createMockAudioBuffer(10));
  });
  ...
  describe('Stem Separation Processing', () => {
    ...
    it('should handle processing errors gracefully', async () => {
      ...
      await expect(processor.processStemSeparation(mockFile))
        .rejects.toThrow('Demucs processing failed: Decode failed');
    });

    it('should handle file read errors', async () => {
      // Mock file.arrayBuffer to throw an error
      const errorFile = createMockFile();
      (errorFile.arrayBuffer as jest.Mock).mockRejectedValue(new Error('File read failed'));

      await expect(processor.processStemSeparation(errorFile))
        .rejects.toThrow('Demucs processing failed: File read failed');
    });
  });
  ...
  describe('Stem Data Validation', () => {
    it('should create valid stem data for each stem type', async () => {
      ...
      const stemTypes = ['drums', 'bass', 'melody', 'vocals', 'original'] as const;
  ...
  describe('Metadata Generation', () => {
    ...
    it('should include error in metadata when processing fails', async () => {
      mockAudioContext.decodeAudioData.mockRejectedValue(new Error('Test error'));
    ...
    it('should record processing time accurately', async () => {
      ...
      expect(result.metadata.processingTime).toBeLessThan(endTime - startTime + 100); // Allow some tolerance
    });
  });
  ...
  describe('File Type Handling', () => {
    ...
    it('should handle files with unusual names', async () => {
      ...
      expect(result.metadata.status).toBe('completed');
      expect(result.metadata.inputFile.name).toBe('file with spaces & symbols!@#.mp3');
    });
  });
  ...
  describe('Edge Cases and Error Conditions', () => {
    it('should handle empty files', async () => {
      ...
      await expect(processor.processStemSeparation(emptyFile))
        .rejects.toThrow();
    });
    ...
    it('should handle corrupted audio data', async () => {
      ...
      await expect(processor.processStemSeparation(mockFile))
        .rejects.toThrow('Demucs processing failed');
    });

    it('should handle very short audio files', async () => {
      // Mock a very short audio buffer
      mockAudioContext.decodeAudioData.mockResolvedValue(createMockAudioBuffer(0.1));
    ...
    it('should handle very long audio files', async () => {
      ...
      mockAudioContext.decodeAudioData.mockResolvedValue(createMockAudioBuffer(3600)); // 1 hour
  ...
  describe('Concurrent Processing', () => {
    it('should handle multiple concurrent processing requests', async () => {
      ...
      const promises = files.map(file => processor.processStemSeparation(file));
