...
import StemVisualizerPanel from '../StemVisualizerPanel';
...
jest.mock('../StemControls', () => {
  return function MockStemControls({ channel, stemType }: any) {
    return <div data-testid={`stem-controls-${channel}-${stemType}`}>StemControls</div>;
  };
});

jest.mock('../StemWaveform', () => {
  return function MockStemWaveform({ channel, stemType }: any) {
    return <div data-testid={`stem-waveform-${channel}-${stemType}`}>StemWaveform</div>;
  };
});
...
describe('StemVisualizerPanel', () => {
  ...
  const mockEnableStemPlayer = jest.fn();
  const mockDisableStemPlayer = jest.fn();
  ...
  describe('Channel Tabs', () => {
    ...
    it('shows stem player status indicators', () => {
      ...
      const ch1Tab = channelTabs.find(tab => tab.textContent?.includes('CH1'));
      const ch2Tab = channelTabs.find(tab => tab.textContent?.includes('CH2'));
  ...
  describe('Content Rendering', () => {
    it('shows no track message when no track loaded', () => {
      mockUseEnhancedDJStore.mockReturnValue({
        decks: mockDecks,
        selectedDeck: 2, // Channel 3 has no track
        stemViewMode: 'individual' as const,
        setSelectedDeck: mockSetSelectedDeck,
        setStemViewMode: mockSetStemViewMode,
        enableStemPlayer: mockEnableStemPlayer,
      ...
      expect(screen.getByText('Load a track to channel 3 to see stem controls')).toBeInTheDocument();
    });

    it('shows stem player disabled message when disabled', () => {
      mockUseEnhancedDJStore.mockReturnValue({
        decks: mockDecks,
        selectedDeck: 1, // Channel 2 has track but stem player disabled
        stemViewMode: 'individual' as const,
        setSelectedDeck: mockSetSelectedDeck,
        setStemViewMode: mockSetStemViewMode,
        enableStemPlayer: mockEnableStemPlayer,
      ...
      expect(screen.getByText('Enable stem player to access advanced controls')).toBeInTheDocument();
    });
    ...
    it('renders stem controls when enabled', () => {
      ...
      expect(screen.getByTestId('stem-controls-0-original')).toBeInTheDocument();
    });
    ...
    it('renders individual waveforms in individual mode', () => {
      ...
      expect(screen.getByTestId('stem-waveform-0-original')).toBeInTheDocument();
    });
    ...
    it('renders combined view in combined mode', async () => {
      ...
      expect(screen.getByText('Combined stem visualization (coming soon)')).toBeInTheDocument();
    });
  ...
  describe('Performance', () => {
    ...
    it('cleans up event listeners on unmount', () => {
      ...
      const { unmount } = render(<StemVisualizerPanel enableKeyboardShortcuts={true} />);
