...
describe("GestureFeedback", () => {
  ...
  it("should render nothing when not active", () => {
    const { container } = render(
      <GestureFeedback mapper={mockMapper} isActive={false} />,
    );
  ...
  it("should render nothing when no feedback state is available", () => {
    const { container } = render(
      <GestureFeedback mapper={mockMapper} isActive={true} />,
    );
  ...
  it("should subscribe to mapper feedback updates on mount", () => {
    render(<GestureFeedback mapper={mockMapper} isActive={true} />);

    expect(mockMapper.on).toHaveBeenCalledWith(
      "feedbackUpdate",
      expect.any(Function),
    );
  });
  ...
  it("should unsubscribe from mapper feedback updates on unmount", () => {
    ...
    expect(mockMapper.off).toHaveBeenCalledWith(
      "feedbackUpdate",
      expect.any(Function),
    );
  });
  ...
  it("should show channel information", () => {
    ...
    render(<GestureFeedback mapper={mockMapper} isActive={true} channel={2} />);
  ...
  it("should handle empty gesture state", () => {
    ...
    expect(
      screen.getByText("Start making gestures to see them here"),
    ).toBeInTheDocument();
  });
