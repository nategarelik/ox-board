...
describe('StemControls', () => {
  ...
  describe('EQ Controls', () => {
    it('adjusts EQ values when knobs are dragged', async () => {
      ...
      const highKnob = screen.getByText('HIGH').closest('div')?.querySelector('[role="button"]') as HTMLElement;
      ...
      await waitFor(() => {
        expect(mockSetStemEQ).toHaveBeenCalledWith(0, 'bass', 'high', expect.any(Number));
      });
    });
    ...
    it('resets EQ to 0 on double click', async () => {
      ...
      const midKnob = screen.getByText('MID').closest('div')?.querySelector('[role="button"]') as HTMLElement;
    ...
    it('clamps EQ values between -20 and +20', async () => {
      ...
      const lowKnob = screen.getByText('LOW').closest('div')?.querySelector('[role="button"]') as HTMLElement;
  ...
  describe('Pan Control', () => {
    it('adjusts pan value when pan knob is dragged', async () => {
      ...
      const panKnob = screen.getByText('PAN').closest('div')?.querySelector('[role="button"]') as HTMLElement;
      ...
      await waitFor(() => {
        expect(mockSetStemPan).toHaveBeenCalledWith(0, 'vocals', expect.any(Number));
      });
    });
    ...
    it('displays pan values correctly', () => {
      ...
      const { rerender } = render(<StemControls channel={0} stemType="melody" />);
  ...
  describe('Performance', () => {
    it('uses React.memo for optimization', () => {
      const { rerender } = render(<StemControls channel={0} stemType="drums" />);
    ...
    it('cleans up event listeners on unmount', () => {
      ...
      const { unmount } = render(<StemControls channel={0} stemType="vocals" />);
